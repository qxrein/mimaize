"""
ABC-Q core: Artificial Bee Colony over per-layer bit-widths with calibration fitness.

SENSITIVITY PRIOR — implementation note
---------------------------------------
The sensitivity prior uses a hard threshold: layers in the top quartile of normalized gradient
magnitude (s̃_i >= Q_0.75) are never initialized to 2 bits, and mutation probability for dimension d
is proportional to (1 - s̃_d). The parameter ``Q_THRESHOLD`` controls this quantile.
(Paper Table 1 reference: Q_threshold = 0.75; update paper text to match code, not phi3 notation.)
...
- **Backbone**: ``BACKBONE`` in ``resnet20``, ``mobilenetv2``, ``efficientnetb0``. Nested backbones use
  **unique layer names** for quant targets.
- **Dataset**: ``DATASET`` in ``cifar10``, ``cifar100`` (same 32×32 input; ``NUM_CLASSES`` 10 or 100).
- **Seeds**: Use ``N_SEEDS`` and ``SEEDS``. Override output dir with env ``ABC_Q_RESULTS_DIR`` (default
  ``results/<BACKBONE>/<DATASET>/`` under this package).
- **Dry run**: Set env ``ABC_Q_DRY_RUN=1`` (or use ``python master_run.py --dry-run``) for minimal
  epochs, single seed, and reduced search budget — for pipeline checks only; outputs go under
  ``results/_dry_run/...`` when driven by ``master_run.py``.
"""
...
from abc_q_plots import save_all_paper_figures
from abc_q_stats import paired_mean_std, print_paper_table, wilcoxon_paired_pvalue

_ABC_Q_ROOT = os.path.dirname(os.path.abspath(__file__))


def _env_dry_run() -> bool:
    """True when ``ABC_Q_DRY_RUN`` is set (minimal epochs / search for pipeline smoke tests)."""
    return os.environ.get("ABC_Q_DRY_RUN", "").strip().lower() in ("1", "true", "yes", "on")


def configure_gpu_memory() -> None:
    """Enable memory growth to prevent full VRAM pre-allocation."""
...
# --- Entry points override these after ``from abc_q_core import *`` ---
...
Q_THRESHOLD = 0.75  # sensitivity quartile floor (paper Table 1)

RUN_CYCLE_ABLATION = False
ABLATION_CYCLES = [10, 15, 20, 25, 30]
ABLATION_SEED = 42
ACTIVATION_COLLECT_BATCH = 64  # safer than 128 on 4GB VRAM
...
def get_experiment_config() -> Dict[str, Any]:
    ...
    if FULL_DATASET:
        ...
        act_batch = 64 if BACKBONE == "mobilenetv2" else 128
        cfg = {
            "train_samples": 49000,
            "calib_samples": 1000,
            "test_samples": 10000,
            ...
            "scout_limit": 8,
            "batch_size": 128,
            "activation_collect_batch": act_batch,
        ...
        cfg = {
            ...
            "scout_limit": 8,
            "batch_size": 64,
            "activation_collect_batch": 64 if BACKBONE == "mobilenetv2" else 128,
            "pareto_random_samples": 500,
            "use_sensitivity_prior": True,
            "run_sensitivity_ablation": True,
            "backbone": BACKBONE,
    ...
    if dry:
        cfg["pretrain_epochs"] = min(int(cfg["pretrain_epochs"]), 1)
        cfg["finetune_epochs"] = 1
        cfg["abc_cycles"] = 2
        cfg["num_bees"] = 4
        cfg["scout_limit"] = 2
        cfg["pareto_random_samples"] = min(int(cfg["pareto_random_samples"]), 50)
        cfg["run_sensitivity_ablation"] = False
    return cfg


def load_dataset_splits(config: Dict[str, Any], data_split_seed: int = DATA_SPLIT_SEED) -> Dict[str, np.ndarray]:
    """Shuffle CIFAR-10 or CIFAR-100 once, then slice train / calib / test per config."""
    if DATASET == "cifar100":
...
def build_efficientnetb0_cifar(
    num_classes: Optional[int] = None,
    learning_rate: Union[float, tf.keras.optimizers.schedules.LearningRateSchedule] = 1e-3,
) -> Model:
    """EfficientNet-B0 (32×32, no ImageNet weights) + GAP + dense logits + softmax."""
    if num_classes is None:
        num_classes = int(NUM_CLASSES)
...
def build_model(
    ...
    """Dispatch CIFAR model by ``BACKBONE``."""
    if num_classes is None:
        num_classes = int(NUM_CLASSES)
    if BACKBONE == "mobilenetv2":
    ...
    if BACKBONE == "efficientnetb0":
        return build_efficientnetb0_cifar(num_classes, learning_rate)
    raise ValueError(f"Unknown BACKBONE={BACKBONE!r}; use 'resnet20', 'mobilenetv2', or 'efficientnetb0'.")
...
def collect_activation_stats(
    ...
    x_calib: np.ndarray,
    y_calib: np.ndarray,
    batch_size: int = ACTIVATION_COLLECT_BATCH,
...
# The sensitivity prior uses a hard threshold: layers in the top quartile of normalized gradient
# magnitude (s̃_i >= Q_0.75) are never initialized to 2 bits, and mutation probability for dimension d
# is proportional to (1 - s̃_d). The parameter Q_THRESHOLD below controls this quantile.
# (Paper Table 1 reference: Q_threshold = 0.75)
...
def initialize_food_sources(
    ...
    threshold = np.quantile(sensitivity, Q_THRESHOLD)
...
def run_abc_q(
    ...
    best.search_wall_seconds = float(time.perf_counter() - _t0)
    restore_original_weights(model, quant_names, original_weights)
...
def log_layer_bit_frequency(
    all_best_configs: List[np.ndarray],
    quant_names: List[str],
    out_path: str,
) -> None:
    """Per-layer modal bit-width across seeds. Writes CSV."""

    n_seeds = len(all_best_configs)
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    with open(out_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["layer_idx", "layer_name", "frac_2bit", "frac_4bit", "frac_8bit", "modal_bit", "mean_bits"]
        )
        for li, name in enumerate(quant_names):
            chosen = [int(cfg[li]) for cfg in all_best_configs]
            row = [
                li,
                name,
                round(chosen.count(2) / n_seeds, 3),
                round(chosen.count(4) / n_seeds, 3),
                round(chosen.count(8) / n_seeds, 3),
                max([2, 4, 8], key=chosen.count),
                round(sum(chosen) / n_seeds, 2),
            ]
            writer.writerow(row)
    print(f"[log] Layer bit frequency saved to {out_path}")


def run_cycle_ablation(
    model: Model,
    quant_names: List[str],
    x_calib: np.ndarray,
    y_calib: np.ndarray,
    param_counts: np.ndarray,
    sensitivity: np.ndarray,
    layer_bops_specs: List[Dict[str, Any]],
    activation_stats: Optional[Dict[int, Tuple[float, float]]],
    results_dir: str,
) -> None:
    """Run ABC-Q at different T values. Single seed, MobileNetV2."""
    if not RUN_CYCLE_ABLATION:
        return

    set_seed(ABLATION_SEED)
    rows: List[List[Any]] = []
    cfg = get_experiment_config()
    for T in ABLATION_CYCLES:
        print(f"[ablation] T={T}")
        result, _ = run_abc_q(
            model=model,
            ...
            num_bees=cfg["num_bees"],
            cycles=T,
            scout_limit=cfg["scout_limit"],
            layer_bops_specs=layer_bops_specs,
            activation_stats=activation_stats,
            quantize_activations=QUANTIZE_ACTIVATIONS,
            use_sensitivity_prior=True,
        )
        rows.append([T, round(result.accuracy, 4), round(result.bops_ratio, 4), round(result.search_wall_seconds, 1)])
    out = os.path.join(results_dir, "cycle_ablation.csv")
    with open(out, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["T", "calib_acc", "bops_ratio", "wall_sec"])
        w.writerows(rows)
    print(f"[ablation] Results saved to {out}")


def main() -> None:
    """Train float model, run searches across seeds, print table, save figures under ``results/<BACKBONE>/<DATASET>/``."""
    configure_gpu_memory()
    global N_SEEDS, PRETRAIN_EPOCHS_OVERRIDE, RUN_CYCLE_ABLATION
    if _env_dry_run():
        print(
            "[dry-run] ABC_Q_DRY_RUN=1: minimal pretrain, single seed, reduced ABC/search "
            "(see get_experiment_config). No sensitivity ablation; no cycle ablation."
        )
        N_SEEDS = 1
        PRETRAIN_EPOCHS_OVERRIDE = 1
        RUN_CYCLE_ABLATION = False
    ...
    print(
        f"BACKBONE={BACKBONE} DATASET={DATASET} NUM_CLASSES={NUM_CLASSES} FULL_DATASET={FULL_DATASET} "
        f"N_SEEDS={N_SEEDS} QUANTIZE_ACTIVATIONS={QUANTIZE_ACTIVATIONS} REAL_BOPS={REAL_BOPS} seeds={seeds}"
    )
    ...
    for si, s in enumerate(seeds):
        ...
        abc_wall.append(float(abc_best.search_wall_seconds))
        ...
        rand_wall.append(float(random_best.search_wall_seconds))
        ...
        cma_wall.append(float(cma_best.search_wall_seconds))
    ...
    log_layer_bit_frequency(
        all_best_configs=[r.bit_config for r in seed_results],
        quant_names=list(quant_names),
        out_path=os.path.join(results_dir, "layer_bit_freq.csv"),
    )
    ...
    w_abc_m, w_abc_s = _agg(abc_wall)
    w_rand_m, w_rand_s = _agg(rand_wall)
    w_cma_m, w_cma_s = _agg(cma_wall)
    print("\nMethod\t\tMean search wall (s)\tStd (s)")
    print("-" * 60)
    print(f"ABC-Q (ours)\t{w_abc_m:.2f}\t{w_abc_s:.2f}")
    print(f"Random Search\t{w_rand_m:.2f}\t{w_rand_s:.2f}")
    print(f"CMA-ES (Hansen)\t{w_cma_m:.2f}\t{w_cma_s:.2f}")
    ...
    final_model.compile(
        ...
        loss=sparse_cce_with_label_smoothing(int(NUM_CLASSES), LABEL_SMOOTHING),
        metrics=["accuracy"],
    )
    ...
    print(f"Test accuracy after quant + fine-tune (last-seed ABC): {final_test_acc:.4f}")
    ...
    summary_payload: Dict[str, Any] = {
        ...
        "search_wall_seconds": {
            "ABC-Q (ours)": {"mean": w_abc_m, "std": w_abc_s},
            "Random Search": {"mean": w_rand_m, "std": w_rand_s},
            "CMA-ES (Hansen)": {"mean": w_cma_m, "std": w_cma_s},
        },
        "paper_table_rows": {
            k: {
                "mean_acc": v[0],
    ...
    timing_csv = os.path.join(results_dir, "method_search_wall_times.csv")
    with open(timing_csv, "w", newline="", encoding="utf-8") as cf:
        cw = csv.writer(cf)
        cw.writerow(["method", "mean_wall_sec", "std_wall_sec"])
        cw.writerow(["ABC-Q (ours)", f"{w_abc_m:.4f}", f"{w_abc_s:.4f}"])
        cw.writerow(["Random Search", f"{w_rand_m:.4f}", f"{w_rand_s:.4f}"])
        cw.writerow(["CMA-ES (Hansen)", f"{w_cma_m:.4f}", f"{w_cma_s:.4f}"])
    print(f"[log] Search wall times written to {timing_csv}")

    done_path = os.path.join(results_dir, "RUN_SUCCEEDED")
    with open(done_path, "w", encoding="utf-8") as df:
        df.write("ok\n")
    print(f"[log] Marked complete: {done_path}")

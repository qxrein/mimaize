"""
ABC-Q on CIFAR-10 — **ResNet-20 variant** (duplicate of ``abc_q_cifar10_full.py``).

Use this entry point for the smaller backbone (~22 quantizable Conv/Dense layers vs MobileNetV2).
For MobileNetV2 + many seeds, run ``abc_q_cifar10_full.py`` instead.
...
- **This file**: ``BACKBONE="resnet20"``, ``N_SEEDS=3`` by default. Figures/tables default to
  ``results/resnet20/`` (see ``RESNET20_RESULTS_DIR``). Override with env ``ABC_Q_RESULTS_DIR``.
"""
...
_RESNET_ROOT = os.path.dirname(os.path.abspath(__file__))
RESNET20_RESULTS_DIR = os.path.join(_RESNET_ROOT, "results", "resnet20")

# --- Publication toggles (ResNet-20 CIFAR-10) ---
BACKBONE = "resnet20"
MOBILENET_ALPHA = 1.0
FULL_DATASET = True
N_SEEDS = 3
